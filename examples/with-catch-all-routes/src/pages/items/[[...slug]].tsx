import { tw } from 'twind/css';
import NextLink from 'next/link';
import NextBreadcrumbs, { createNextCrumbComponent } from 'breadcrumbs-next';

function Breadcrumbs({ children, ...props }) {
  return (
    <nav {...props} className={tw`flex`}>
      <ol className={tw(`inline-flex items-center space-x-1 md:space-x-3`)}>{children}</ol>
    </nav>
  );
}

const Crumb = createNextCrumbComponent(({ children, href, isLast, isFirst }) => {
  return (
    <li aria-current={isLast ? 'page' : null}>
      <div className={tw`flex items-center`}>
        {!isFirst ? (
          <svg
            aria-hidden="true"
            className={tw`w-6 h-6 text-gray-400`}
            fill="currentColor"
            viewBox="0 0 20 20"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
              fillRule="evenodd"
              d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z"
              clipRule="evenodd"
            />
          </svg>
        ) : (
          <svg
            aria-hidden="true"
            className={tw`w-4 h-4`}
            fill="currentColor"
            viewBox="0 0 20 20"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path d="M10.707 2.293a1 1 0 00-1.414 0l-7 7a1 1 0 001.414 1.414L4 10.414V17a1 1 0 001 1h2a1 1 0 001-1v-2a1 1 0 011-1h2a1 1 0 011 1v2a1 1 0 001 1h2a1 1 0 001-1v-6.586l.293.293a1 1 0 001.414-1.414l-7-7z" />
          </svg>
        )}
        {isLast ? (
          children
        ) : (
          <NextLink
            href={href}
            className={tw`text-sm font-medium text-gray-700 hover:text-blue-600 md:ml-2 dark:text-gray-400 dark:hover:text-white`}
          >
            {children}
          </NextLink>
        )}
      </div>
    </li>
  );
});

export default function ItemsSlug() {
  return (
    <>
      <div className={tw`p-2`}>
        <NextBreadcrumbs Container={Breadcrumbs} Crumb={Crumb} />
      </div>
      ItemsSlug Path
    </>
  );
}
